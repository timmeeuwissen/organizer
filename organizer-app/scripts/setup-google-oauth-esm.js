...
/**
 * Google OAuth Setup Helper Script (ESM Version)
 * 
 * This script guides users through the process of setting up OAuth
 * credentials for Google API integration including Gmail, Calendar, and Contacts.
 */
...
const GOOGLE_OAUTH_SCOPES = [
  ...
  'https://www.googleapis.com/auth/contacts',         // Full access to manage contacts
...
async function main() {
  ...
  console.log('This script will help you set up OAuth credentials for Google API integration.');
  console.log('Includes access to: Gmail, Google Calendar, and Google People/Contacts APIs');
  ...
  if (!credentials) {
    ...
    console.log('   d) Search for and enable the following APIs:');
    console.log('      - Gmail API');
    console.log('      - Google Calendar API');
    console.log('      - People API');
    console.log('   e) Go to APIs & Services > OAuth consent screen');
    console.log('      - Choose "External" user type if this is a personal project');
    console.log('      - Fill in the required app information (app name, user support email, developer contact)');
    console.log('      - Add the scopes for Gmail, Calendar, and People APIs');
    console.log('      - Add your email as a test user');
    console.log('   f) Go to APIs & Services > Credentials');
    console.log('   g) Click Create Credentials > OAuth client ID');
    console.log('   h) Set Application Type to "Web application"');
    console.log('   i) Add "http://localhost:3333/oauth2callback" as an authorized redirect URI');
    console.log('   j) Create the credentials and download the JSON file');
    console.log('   k) Note: For production use, you would need to go through Google verification process');
  ...
  console.log('\n\x1b[33m2. Now we need to authorize the application to access your Google account\x1b[0m');
  ...
  console.log('Your credentials and refresh token have been saved to:');
  console.log('- Credentials:', CREDENTIALS_FILE);
  console.log('- Tokens:', TOKENS_FILE);
  
  console.log('\n\x1b[33mInstructions for setting up the integration in the Organizer App:\x1b[0m');
  console.log('1. Go to the Profile section in the app');
  console.log('2. Navigate to the Integrations tab');
  console.log('3. Click "Add Integration" and select "Google"');
  console.log('4. Enter the following details:');
  console.log('   - Account Name: Choose a recognizable name (e.g., "My Google Account")');
  console.log('   - Client ID:', credentials.installed?.client_id || credentials.web?.client_id);
  console.log('   - Client Secret:', credentials.installed?.client_secret || credentials.web?.client_secret);
  console.log('   - Refresh Token:', authorizationResult.refresh_token);
  console.log('5. Select which services to synchronize (Mail, Calendar, Contacts)');
  console.log('6. Choose display colors if desired');
  console.log('7. Click Save to complete the integration setup');
  ...
  console.log('- Keep your client secret and refresh token secure');
  console.log('- If you encounter issues, you may need to regenerate the refresh token');
  console.log('- For production use, verify your application with Google to remove "unverified app" warnings');
  
  console.log('\n\x1b[32mSetup complete! You can now use Google API integrations in the Organizer App.\x1b[0m');
  rl.close();
}
